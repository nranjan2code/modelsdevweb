        ...
        <table className="table-base min-w-[560px]">
          ...
          <tbody>
            {catalog.providers.map((p) => (
              <tr key={p.id}>
                <td>
                  <Link href={`/provider/${p.id}`} className="font-medium text-black transition-colors hover:text-accent">
                    {p.name}
                  </Link>
                  <span className="ml-2 font-mono text-xs text-black/45">{p.id}</span>
                </td>
                <td className="text-right tabular-nums text-black/60">{p.modelCount}</td>
                <td className="font-mono text-xs text-black/60">{p.npm ?? "—"}</td>
